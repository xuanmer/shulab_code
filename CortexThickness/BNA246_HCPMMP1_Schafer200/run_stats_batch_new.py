...
HEMISPHERES = ["lh", "rh"]
# 只将指定的测量值添加到Excel中
MEASUREMENTS = ["area", "volume", "thickness"]
# 所有测量值用于CSV生成
ALL_MEASUREMENTS = ["area", "volume", "thickness", "meancurv", "gauscurv", "foldind", "curvind"]
...
SUBCORTICAL_STATS = [
    {
        ...
        "file": "aseg.stats",
        "output_dir": "aparc",
        "excel_name": "aseg_vol_stats.xlsx",
        "sheet_name": "aseg.vol.table"
    },
    {
...
def run_aparcstats2table_parallel(subj_count, parc_dirs, max_workers=8):
    ...
    tasks = []
    total = len(HEMISPHERES) * len(ALL_MEASUREMENTS) * len(PARCELLATIONS)
    idx = 1
...
def run_asegstats2table(subj_count, parc_dirs):
    ...
    for stats_info in SUBCORTICAL_STATS:
        ...
        if result.returncode == 0:
            ...
            # 处理aseg.volume.csv: 重命名文件和列名
            if stats_name == "aseg":
                new_output_file = os.path.join(output_dir, f"{stats_name}.vol.csv")
                try:
                    os.rename(output_file, new_output_file)
                    print(f"[INFO] Renamed {output_file} to {new_output_file}")
                    
                    # 读取并修改列名
                    df = pd.read_csv(new_output_file)
                    # 将列名中的"-"替换为"."
                    df.columns = [col.replace('-', '.') for col in df.columns]
                    # 保存修改后的文件
                    df.to_csv(new_output_file, index=False)
                    print(f"[INFO] Modified column names in {new_output_file}")
                except Exception as e:
                    print(f"[FAIL] Error processing {output_file}: {str(e)}")
                    write_log(f"Error processing {output_file}: {str(e)}")
        else:
...
def process_bn_atlas_table(df):
    """处理BN_Atlas表格，删除eTIV列后的正则表达式'lh*R*'和'rh*L*'列"""
    if 'eTIV' not in df.columns:
        return df
    
    # 找到eTIV列的索引
    etiv_idx = df.columns.get_loc('eTIV')
    
    # 构建正则表达式匹配模式，同时匹配 lh*R* 和 rh*L*
    pattern = r'^lh.*R.*$|^rh.*L.*$'
    
    # 筛选出需要删除的列
    cols_to_drop = []
    for col in df.columns[etiv_idx+1:]:
...
def merge_parc_to_excel(parc):
    ...
    # 用于跟踪sheet索引，确保aseg.vol.table作为aparc.xlsx的第二个sheet
    sheet_index = 0
    ...
    # 处理aseg.vol.csv，作为aparc.xlsx的第二个sheet
    if parc == "aparc" and 'aseg.vol.csv' in os.listdir(parc_dir):
        file_path = os.path.join(parc_dir, "aseg.vol.csv")
        sheet_name = "aseg.vol.table"
        try:
            df = pd.read_csv(file_path)
            if not df.empty:
            ...
            for r in dataframe_to_rows(df, index=False, header=True):
                ws.append(r)
            print(f"[OK] Added aseg.vol.csv to Excel as second sheet: {sheet_name}")
            sheet_index += 1
        except Exception as e:
    ...
    for file in os.listdir(parc_dir):
        if file.endswith('.csv'):
            # 跳过Global.csv和aseg.vol.csv
            if file in ['Global.csv', 'aseg.vol.csv']:
                continue
            ...
            if parc == "BN_Atlas":
                try:
                    df = pd.read_csv(file_path)
                    if not df.empty:
                        df = process_bn_atlas_table(df)
                        df.iloc[:, 0] = df.iloc[:, 0].apply(lambda x: os.path.basename(os.path.dirname(x)))
                    ws = wb.create_sheet(title=sheet_name, index=sheet_index)
                    for r in dataframe_to_rows(df, index=False, header=True):
                        ws.append(r)
                    print(f"[OK] Added and processed: {file_path} → {sheet_name}")
                    sheet_index += 1
                except Exception as e:
                    print(f"[FAIL] {file_path} failed: {str(e)}")
            else:
...
def generate_euler_number_excel():
    """
    生成euler.xlsx文件，包含eid, euler_number_l, euler_number_r三列
    eid从aseg.vol.csv中提取，euler_number计算为2-2*SurfaceHoles
    """
    print("[INFO] Generating euler.xlsx ...")
    aseg_vol_csv = os.path.join(CODE_DIR, "aparc", "aseg.vol.csv")
    
    if not os.path.exists(aseg_vol_csv):
        print(f"[FAIL] aseg.vol.csv not found: {aseg_vol_csv}")
        return
    
    try:
        # 读取aseg.vol.csv
        df = pd.read_csv(aseg_vol_csv)
        
        # 提取eid列（假设第一列是路径，提取最后一级目录作为eid）
        if not df.empty:
            df['eid'] = df.iloc[:, 0].apply(lambda x: os.path.basename(os.path.dirname(x)))
        else:
            print("[WARN] aseg.vol.csv is empty")
            return
        
        # 确保存在所需列
        required_cols = ['lhSurfaceHoles', 'rhSurfaceHoles', 'eid']
        if not all(col in df.columns for col in required_cols):
            missing_cols = [col for col in required_cols if col not in df.columns]
            print(f"[FAIL] Missing columns in aseg.vol.csv: {missing_cols}")
            return
        
        # 计算euler数
        df['euler_number_l'] = 2 - 2 * df['lhSurfaceHoles']
        df['euler_number_r'] = 2 - 2 * df['rhSurfaceHoles']
        
        # 选择需要的列
        result_df = df[['eid', 'euler_number_l', 'euler_number_r']]
        
        # 保存为Excel
        euler_excel = os.path.join(CODE_DIR, "euler.xlsx")
        result_df.to_excel(euler_excel, index=False)
        print(f"[OK] euler.xlsx saved: {euler_excel}")
        
    except Exception as e:
        print(f"[FAIL] Error generating euler.xlsx: {str(e)}")
        write_log(f"Error generating euler.xlsx: {str(e)}")
...
def main():
    ...
    Main entry point. Orchestrates the full workflow of:
      ...
      7. Generating euler number Excel file.
