...
tmp2/
  ...
  │     ├── lh.HCP-MMP1.annot
  │     ├── rh.HCP-MMP1.annot
  │     ├── lh.Schaefer2018_200Parcels_Kong2022_17Networks_order.annot
  │     ├── rh.Schaefer2018_200Parcels_Kong2022_17Networks_order.annot
  └── data/
        ├── 1000037/FreeSurfer/
        ├── 1000043/FreeSurfer/
        └── ... (subject folders)
...
MAX_THREADS = 32  # Set this according to your CPU cores

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
CODE_DIR = os.path.join(PROJECT_ROOT, "code")
DATA_DIR = os.path.join(PROJECT_ROOT, "data_1")
...
# -------------------- Schaefer200 atlas file paths --------------------
SCHAEFER200_LH_ANNOT = os.path.join(CODE_DIR, "lh.Schaefer2018_200Parcels_Kong2022_17Networks_order.annot")
SCHAEFER200_RH_ANNOT = os.path.join(CODE_DIR, "rh.Schaefer2018_200Parcels_Kong2022_17Networks_order.annot")
...
def process_subject(sub_id):
    ...
    try:
        ...
        # ------ Schaefer200 ------
        subprocess.run(cmd_schaefer200_lh, env=env, check=True)
        subprocess.run(cmd_schaefer200_rh, env=env, check=True)
        subprocess.run(cmd_schaefer200_lh_stats, env=env, check=True)
        subprocess.run(cmd_schaefer200_rh_stats, env=env, check=True)
