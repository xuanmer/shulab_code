...
# 配置
INPUT_DIR="/media/shulab/Getea/zhe2/NC/tbss/MD"
...
MAX_JOBS=20  # 最大并发任务数（建议设为CPU核心数）
...
# 遍历所有MD文件
find "$INPUT_DIR" -maxdepth 1 -name "*.nii.gz" | while read -r md_file; do
    md_base=$(basename "$md_file" .nii.gz)
    masked_file="${INPUT_DIR}/${md_base}_masked.nii.gz"
    skeleton_file="${INPUT_DIR}/${md_base}_skeletonised.nii.gz"
    stats_file="${INPUT_DIR}/${md_base}_JHUrois.txt"
    # 后台执行处理任务
    {
        echo "开始处理: $md_file"
        command="fslmaths $md_file -mas $MASK1 $masked_file"
        echo "执行命令: $command"
        eval $command
